...
# ══════════════════════════════════════════════════════════════════════════
#  FAST PATH: extract every shot's features ONCE, then match in memory.
#  Equivalent to get_close_fs_pairs IFF geedal_utils.scale_features uses a
#  FIXED scaler (e.g. the standard_scaler.csv), not a per-call fit_transform.
#  ~10-50x faster: replaces N x (2-5) EE calls with a handful of batched ones.
# ══════════════════════════════════════════════════════════════════════════
def precompute_features(shots_gdf, dates, use_baseline=True, use_slope=False,
                        use_embeddings=False):
    """Extract EE features for EVERY shot once (batched, chunked, threaded by
    extract_ee_values). Returns (gdf_with_features, feature_column_list)."""
    geedal_utils.earthengine_init()
    g = shots_gdf.copy().reset_index(drop=True)
    feats = []

    if use_baseline:
        g = geedal_utils_on_steroids.extract_ee_values(
            g, 'COPERNICUS/S1_GRD', dates, agg_fun='median', scale=25,
            band_name=['VV', 'VH'], s1_corr=True)
        feats += ['VV', 'VH']
        s2_bands = ['B' + str(i + 1) for i in range(9)] + ['B' + str(i + 11) for i in range(2)]
        g = geedal_utils_on_steroids.extract_ee_values(
            g, 'COPERNICUS/S2_SR_HARMONIZED', dates, agg_fun='median', scale=25,
            band_name=s2_bands, cloud_free=True)
        feats += s2_bands

    if use_slope:
        g = geedal_utils_on_steroids.extract_ee_values(
            g, 'COPERNICUS/DEM/GLO30', dates=None, agg_fun='mean', scale=25,
            band_name='DEM', slope=True, column_name='slope')
        g = geedal_utils_on_steroids.extract_ee_values(
            g, 'COPERNICUS/DEM/GLO30', dates=None, agg_fun='mean', scale=25,
            band_name='DEM', aspect=True, column_name='aspect')
    ...
    if use_embeddings:
        emb = ['A' + "{:02d}".format(i) for i in range(64)]
        g = geedal_utils_on_steroids.extract_ee_values(
            g, 'GOOGLE/SATELLITE_EMBEDDING/V1/ANNUAL', dates, scale=25, band_name=emb)
        feats += emb

    # seasonality (same encoding as the reference)
    g['seasonality'] = np.cos((2 * np.pi / 12) * (pd.to_datetime(g['time']).dt.month - 6))
    feats += ['seasonality']
    return g, feats


def get_close_fs_pairs_fast(feat_gdf, feat_cols, shots_2,
                            crs='EPSG:25830', max_distance=250, weight_geo=0.75,
                            idx='shot_number', disturbed=True, use_baseline=True,
                            use_slope=False, sel_feats=None, all_feats=False):
    """
    Faithful in-memory version of get_close_fs_pairs: features are extracted ONCE
    (in feat_gdf) instead of per query, but the coordinate handling, query-point
    location, temporal filter, scaling and 1-NN mirror the original exactly.

    feat_gdf : ALL candidate shots WITH feature columns (the old `ds`, as a gdf)
    feat_cols: feature names from precompute_features
    shots_2  : query shots (must have shot_num_2 + geometry, like the original)
    """
    if sel_feats is not None:
        feat_cols = list(sel_feats)

    ds = feat_gdf.copy().reset_index(drop=True)

    # --- candidate coords: SAME as original (Transformer on longitude/latitude) ---
    trans = Transformer.from_crs('EPSG:4326', crs, always_xy=True)
    x, y = trans.transform(ds['longitude'].values, ds['latitude'].values)
    coords = np.array([(x[i], y[i]) for i in range(len(x))])
    tree_coords = cKDTree(coords)

    # --- query coords: SAME as original (shots_2.to_crs) ---
    shots_2_crs = shots_2.to_crs(crs)

    # scale ALL candidate features once (fixed scaler -> == per-query scaling)
    if use_baseline or use_slope:
        Fds = geedal_utils.scale_features(ds[feat_cols].copy())
    else:
        Fds = ds[feat_cols].copy()
    Fds = Fds.to_numpy(dtype=float)

    ids_all   = ds[idx].values
    times_all = pd.to_datetime(ds['time'])
    season_all = np.cos((2*np.pi/12) * (times_all.dt.month.values - 6)) \
        if (sel_feats is None or 'seasonality' in feat_cols) else None
    ...
    def process_one(ind):
        ...
        coords_close = coords[close_pairs]
        close_pairs = list(ds.iloc[close_pairs][idx].values)

        # locate the query shot within the candidate list (as in the original)
        try:
            s2_idx = close_pairs.index(np.uint64(shots_2.iloc[ind].shot_num_2))
        except ValueError:
            return None, [], None, None, None, None

        pos = ds.index[ds[idx].isin(close_pairs)].to_numpy()   # positions of candidates
        # keep alignment with close_pairs order
        order = {sid: p for p, sid in zip(ds.index.values, ds[idx].values)}
        pos = np.array([order[s] for s in close_pairs])

        feats = list(feat_cols)

        # --- query (s2) and candidate feature matrices from the ONE-TIME scaling ---
        s2 = Fds[pos[s2_idx]].astype(float).copy()             # scaled query features
        possible = Fds[pos].astype(float).copy()               # scaled candidate features

        # temporal filter — identical logic to the original
        cand_times = times_all.iloc[pos].to_numpy()
        if disturbed:
            fire_date = shots_2.iloc[ind]['dist']
            mask = cand_times < np.datetime64(pd.Timestamp(fire_date))
        else:
            mask = cand_times < cand_times[s2_idx]
        if not mask.any():
            return None, [], None, None, None, None

        close_pairs = np.array(close_pairs)[mask]
        coords_close = coords_close[mask]
        possible = possible[mask]

        # standardise coords per candidate set, append weighted (as original)
        scaler = StandardScaler()
        pc = scaler.fit_transform(coords_close)
        s2c = scaler.transform(np.array(pair))[0]

        possible = np.hstack([possible, pc])                   # [...feats, X, Y]
        s2 = np.hstack([s2, s2c])

        Xw = np.hstack([possible[:, :-2] * (1 - weight_geo),
                        possible[:, -2:] * weight_geo])
        s2w = np.hstack([s2[:-2] * (1 - weight_geo),
                         s2[-2:] * weight_geo])

        fs_tree = cKDTree(Xw)
        fs_dist, j = fs_tree.query(s2w)

        names = list(feats) + ['X', 'Y']
        s2_dict = {n: {0: float(v)} for n, v in zip(names, s2)}
        s1_dict = {n: {0: float(v)} for n, v in zip(names, possible[j])}
        allf = ({n: {i: float(possible[i, c]) for i in range(len(possible))}
                 for c, n in enumerate(names)} if all_feats else None)

        return (str(close_pairs[j]), [str(c) for c in close_pairs],
                s2_dict, s1_dict, allf, float(fs_dist))

    results = [process_one(i) for i in range(len(shots_2))]
    new_inds, poss_inds, s2f, s1f, allp, md = zip(*results) if results else \
        ([], [], [], [], [], [])

    out = shots_2.copy().reset_index(drop=True)
    out['new_shot_num_1'] = list(new_inds)
    out['possible_pairs'] = list(poss_inds)
    out['s2_feats']       = list(s2f)
    out['s1_feats']       = list(s1f)
    out['all_pos_feats']  = list(allp)
    out['match_dist']     = list(md)
    return out